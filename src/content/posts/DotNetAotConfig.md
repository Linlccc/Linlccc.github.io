...
updated: '2026-03-25T17:03:08.761+08:00'
---
...
- **适用值**：`true` / `false`（默认）。
- **说明**：这是最为激进的终极体积优化开关。设为 `true` 后，任何调用 `System.Reflection` 的代码都会抛出异常，序列化（如 JSON）和依赖注入必须 100% 采用源码生成器（Source Generators）。

### `IlcScanReflection`

- **作用**：指示编译器在编译期是否主动扫描代码中的反射模式以保留类型元数据。
- **适用值**：`true`（默认） / `false`。
- **说明**：配合彻底禁用反射时，设为 `false` 可大幅减少编译耗时与元数据残留。

### `IlcGenerateStackTraceData`

- **作用**：控制是否在二进制文件中嵌入异常堆栈的方法名数据。
- **适用值**：`true`（默认） / `false`。
...
- **作用**：控制是否启用 CFG（Control Flow Guard）控制流保护，防止内存损坏漏洞。
- **适用值**：`Guard` / 留空（默认禁用）。
- **限制**：仅限 **Windows** 平台。
...
- **作用**：控制是否启用全球化不变模式。
- **适用值**：`true` / `false`（默认）。
- **说明**：设为 `true` 将移除对 ICU 的依赖，体积显著减小。但应用将退化为仅支持“不变文化”（英语），无法处理特定国家语言、日期或货币格式。

#### `HybridGlobalization` （.NET 8+）

- **作用**：控制是否启用混合全球化模式。
- **适用值**：`true` / `false`（默认）。
- **说明**：设为 `true` 时调用操作系统原生全球化 API，避免打包庞大的 ICU 数据。
...
- **作用**：调试器附加支持。
- **适用值**：`true`（默认） / `false`。
- **说明**：设为 `false` 将移除调试器支持代码。

#### `EventSourceSupport`

- **作用**：基于 `EventSource` 的日志记录 (ETW/EventPipe) 支持。
- **适用值**：`true`（默认） / `false`。
- **说明**：无需复杂遥测的生产环境可设为 `false`。

#### `MetricsSupport`

- **作用**：`System.Diagnostics.Metrics`（如 OpenTelemetry 指标）支持。
- **适用值**：`true`（默认） / `false`。

#### `HttpActivityPropagationSupport`

- **作用**：`HttpClient` 的分布式链路追踪支持。
- **适用值**：`true`（默认） / `false`。

#### `StackTraceSupport` （.NET 8+）

- **作用**：异常抛出时的堆栈跟踪字符串格式化支持。
- **适用值**：`true`（默认） / `false`。
...
- **作用**：控制是否使用针对体积优化过的 LINQ 实现。
- **适用值**：`true` / `false`（默认）。
- **说明**：设为 `true` 时会牺牲少许运行速度，换取更小的程序集体积。

#### `UseSystemResourceKeys` （.NET 8+）

- **作用**：控制是否使用简短的资源键名（如 `Arg_NullReferenceException`）来替代框架抛出的长篇异常提示说明。
- **适用值**：`true` / `false`（默认）。
- **说明**：设为 `true` 可节省数百 KB 包含长字符串的资源体积。
...
- **作用**：HTTP/3 协议的底层代码支持。
- **适用值**：`true`（默认） / `false`。
- **说明**：不需要 HTTP/3 时设为 `false` 可移除相关网络代码。

#### `UseNativeHttpHandler`

- **作用**：控制是否使用操作系统的原生 HTTP 处理器。
- **适用值**：`true` / `false`（默认）。
- **说明**：设为 `true` 可裁剪掉 .NET 庞大的 `SocketsHttpHandler` 托管实现代码。
- **限制**：仅限 **WASM** 和 **移动端（iOS/Android/MacCatalyst）** 平台。

#### `XmlResolverIsNetworkingEnabledByDefault`

- **作用**：控制 XML 解析器默认是否允许网络访问。
- **适用值**：`true`（默认） / `false`。
- **说明**：设为 `false` 有助于裁剪底层的网络依赖模块。
...
- **作用**：控制是否启用不安全的 UTF-7 编码支持。
- **适用值**：`true` / `false`（较新 .NET 版本中默认已为 false）。

#### `EnableUnsafeBinaryFormatterSerialization`

- **作用**：控制是否启用高危的 `BinaryFormatter` 序列化组件支持。
- **适用值**：`true` / `false`（.NET 8+ 中默认已为 false）。

#### `StartupHookSupport`

- **作用**：通过环境变量向程序注入代码的启动钩子（Startup Hook）支持。
- **适用值**：`true`（默认） / `false`。

#### `MetadataUpdaterSupport`

- **作用**：Hot Reload（热重载）相关的元数据更新支持。
- **适用值**：`true`（默认） / `false`。
- **说明**：生产环境 AOT 无需此功能，可设为 `false` 减小体积。

#### `AutoreleasePoolSupport` （.NET 8+）

- **作用**：隐式 `NSAutoreleasePool` 管理支持。
- **适用值**：`true`（默认） / `false`。
- **说明**：设为 `false` 可禁用该隐式管理。
...
- **作用**：指示当前程序集是否为“裁剪安全”。
- **适用值**：`true` / `false`（默认）。
- **说明**：在自定义类库的 `.csproj` 中设为 `true`，可向编译器宣告此库支持裁剪。
...
- **作用**：指定 XML 描述文件以精确保留特定的类和方法不被裁剪。
- **适用值**：XML 文件路径（如 `TrimmerRoots.xml`）。
- **说明**：此配置属于 MSBuild 的 **项 (Item)**，需要写在 `<ItemGroup>` 节点中，使用 `Include` 属性指定目标。

### `SuppressTrimAnalysisWarnings`

- **作用**：控制是否屏蔽所有的裁剪警告（如 `IL2026`）。
- **适用值**：`true` / `false`（默认）。
- **说明**：掩盖警告并不能解决运行时崩溃风险，仅用于确认代码绝对安全的情况。

### `TrimmerSingleWarn`

- **作用**：控制是否将每个包含警告的程序集的裁剪警告合并为一条总括性日志输出，以减少控制台噪音。
- **适用值**：`true` / `false`（默认）。
...
- **作用**：控制是否在 IL 裁剪阶段彻底移除程序集内残留的符号元数据。
- **适用值**：`true`（开启裁剪时默认） / `false`。

### `StripSymbols`

- **作用**：控制是否剥离原生二进制文件中的本机调试符号（Native Symbols）。
...
- **作用**：控制是否由开发者自定义 Native Main 入口点（即防止 C# 自动生成程序的执行入口点）。
- **适用值**：`true` / `false`（默认）。
